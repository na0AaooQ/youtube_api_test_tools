#!/usr/bin/env bash
#############################################
# 指定したYouTubeコメントのモデレーション状態を published に変更する検証スクリプトです。
#
# 使用する環境変数:
...
printf 'Publish comment %s? Type YES to continue: ' "$TEST_COMMENT_ID" >&2
read -r confirmation
...
response_file="$(mktemp "${TMPDIR:-/tmp}/youtube-moderation-published.XXXXXX")"
...
if ! http_status="$(curl -sS -o "$response_file" -w '%{http_code}' -G -X POST \
  ...
  --data-urlencode "moderationStatus=published")"; then
  printf 'comments.setModerationStatus request failed.\n' >&2
  exit 1
fi

if [ "$http_status" != "204" ]; then
  printf 'comments.setModerationStatus published failed (HTTP %s).\n' \
    "$http_status" >&2
...
printf 'Success: comment %s set to published (HTTP 204).\n' "$TEST_COMMENT_ID"
